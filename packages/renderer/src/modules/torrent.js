...
class TorrentWorker extends Worker {
  ...
  handleMessage(e) {
    if (e.data.type === "subtitle") {
      // console.log(e);
      if (subs) subs.handleSubtitle(e.data.data);
    }
    if (e.data.type === "fonts") {
      // console.log(e);
      if (subs) subs.handleFonts();
    }
    if (e.data.type === "tracks") {
      // console.log(e);
      if (subs) subs.handleTracks(e.data.data);
    }
    if (e.data.type === "file") {
      // console.log(e);
      if (subs) subs.handleFile(e.data.data);
    }
    if (e.data.type === "torrent") {
      // console.log('Torrent event emitted');
      document.dispatchEvent(
        new CustomEvent("torrentHosting", {
          detail: {
            isDone: "true",
          },
        })
